...
public class LoginActivity extends Activity implements View.OnClickListener, LoginUtils.LoginCallback, View.OnLayoutChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Log.i("lxw", "hadlogin=" + hadlogin + "----userCode=" + userCode + "-----password=" + password + "urlPath=" + urlPath);

        if (hadlogin && !userCode.equals("") && !urlPath.equals("") && !password.equals("")) {
            postUserInfo();
    ...
    private void initView() {
        ...
        userName.setText(msharedPreferences.getString("login_type", ""));
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.login_btn:
                if (ischecklogin()) {
                    ...
                    if (!school_name.getText().toString().equals("")) {
                        try {
                            login.setUrlPath(loginschoolconfig.getLoginServUrl());
                        } catch (Exception e) {
            ...
            case R.id.change_school_layout:
                ...
                startActivityForResult(intent1, NOTICE_START_ACTIVITY_SCHOOL_CODE);
        ...
        if (requestCode == NOTICE_START_ACTIVITY_SCHOOL_CODE) {
            switch (resultCode) {
                case NOTICE_CODE_SCHOOL_ACTIVITY_RESULT:
                    loginschoolconfig = (Login_school_config) intent.getExtras().getSerializable("schoolconfig");
        ...
        try {
            if (msg != null) {
                ...
                if (login_teacherInfo.getResultCode() == 0) {
                    ...
                    //张寓宣 2017-5-9 14:53:40
                    String user_name = userName.getText().toString();
                    msharedPreferences.edit().putString("login_type", user_name).apply();
    ...
    private void getinsystem() {
        ...
        //
        msharedPreferences.edit().putString("userCode", login_teacherInfo.getLogin_userInfos().get(0).getUserCode()).apply();
        msharedPreferences.edit().putString("password", passWord.getText().toString().trim()).apply();
        msharedPreferences.edit().putString("teacherName", login_teacherInfo.getLogin_userInfos().get(0).getTeacherName()).apply();
        msharedPreferences.edit().putString("mobileNum", login_teacherInfo.getLogin_userInfos().get(0).getMobileNum()).apply();
        ...
        if (StringUtils.isEmpty(school_name.getText().toString())) {
            msharedPreferences.edit().putString("LoginServUrl", Constants.BASE_LOGIN_URL).apply();
        } else {
        }
