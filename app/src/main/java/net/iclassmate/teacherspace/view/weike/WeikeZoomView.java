...
public class WeikeZoomView extends View {
    ...
    private float lineStrokeWidth = 6f;  //默认涂鸦画笔宽度
    private float xpStrokeWidth = 40f;    //默认橡皮擦宽度
    ...
    public WeikeZoomView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
    ...
    public Bitmap getImageBitmap() {
        if (currentStatus == STATUS_INIT)//还未植入图片
            return null;
        //①、创建新的空画布
        Bitmap mBitmap = Bitmap.createBitmap(sourceBitmap.getWidth(), sourceBitmap.getHeight(), Bitmap.Config.ARGB_8888);
...
//                    mPath.lineTo(move_x, move_y);//移动
                    float dx = Math.abs(move_x - mX);
                    float dy = Math.abs(mY - move_y);
                    if (dx >= TOUCH_TOLERANCE || dy >= TOUCH_TOLERANCE) {
                        // 从x1,y1到x2,y2画一条贝塞尔曲线，更平滑(直接用mPath.lineTo也是可以的)
                        mPath.quadTo(mX, mY, (move_x + mX) / 2, (move_y + mY) / 2);
                        mX = move_x;
                        mY = move_y;
                    }
                    if (currentStatus == STATUS_TY) {
    ...
    private void initBitmap(Canvas canvas) {
        if (sourceBitmap == null) {
            sourceBitmap = Bitmap.createBitmap(getWidth(), getHeight(), Bitmap.Config.RGB_565);
            Canvas mCanvas = new Canvas(sourceBitmap);
        ...
        if (sourceBitmap != null) {
            ...
            //创建一副原图大小相同的透明背景图片
            bgBitmap = Bitmap.createBitmap(sourceBitmap.getWidth(), sourceBitmap.getHeight(), Bitmap.Config.ARGB_8888);
